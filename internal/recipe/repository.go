...
func (r *pgRecipeRepository) FindByID(ctx context.Context, userID, id string) (*Recipe, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("recipeRepo.FindByID: ingredients: %w", err)
	}
...
func (r *pgRecipeRepository) List(ctx context.Context, userID string) ([]Recipe, error) {
	...
	for _, row := range rows {
		recipes = append(recipes, *toRecipe(row))
	}
	return recipes, nil
}
...
func (r *pgRecipeRepository) Delete(ctx context.Context, userID, id string) error {
	if err := r.queries.DeleteRecipe(ctx, db.DeleteRecipeParams{ID: id, UserID: userID}); err != nil {
		return fmt.Errorf("recipeRepo.Delete: %w", err)
	}
	return nil
}
